...
/**
  ...
  **/
class MusicPlayerImplSpec extends AsyncFlatSpec with Matchers {

  def musicPlayer(maybePlaylist: Option[String] = None) =
    new MusicPlayerImpl(new StaticCommandService(maybePlaylist), LowerCasingSynonymService, RemoveBracketsOnlyPunctuationService)
  ...
  it should "eventually parse commands into pairs of strings" in {
    musicPlayer().execute("players 0").map { responses =>
      responses should equal(Seq(
        "count" -> "3",
        "playerindex" -> "0",
        "playerid" -> "00:01:02:03:04:05",
        "uuid" -> "",
        "ip" -> "192.168.1.119:32372",
        "name" -> "Kitchen",
        "seq_no" -> "0",
        "model" -> "squeezebox2",
        "modelname" -> "Squeezebox2",
        "power" -> "1",
        "isplaying" -> "1",
        ...
        "firmware" -> "137",
        "playerindex" -> "1",
        "playerid" -> "40:41:42:43:44:45",
        "uuid" -> "",
        "ip" -> "192.168.1.148:17827",
        "name" -> "Lounge",
        "seq_no" -> "0",
        "model" -> "squeezebox",
        ...
        "firmware" -> "40",
        "playerindex" -> "2",
        "playerid" -> "80:81:82:83:84:85",
        "uuid" -> "",
        "ip" -> "192.168.1.81:41304",
        "name" -> "Bedroom",
        "seq_no" -> "0",
        "model" -> "squeezebox3",
  ...
  it should "eventually get a list of all known players" in {
    musicPlayer().connectedRooms.map { players =>
      players.toSeq should contain only(
  ...
  it should "eventually get a list of all known favourites" in {
    musicPlayer().favourites.map { favourites =>
  ...
  it should "report that nothing is playing" in {
    musicPlayer(Playlists.nothing).playlistInformation(room).map { maybePlaylistInformation =>
      maybePlaylistInformation should equal(None)
    }
  }

  it should "report that that Jimi Hendrix is playing on Planet Rock" in {
    musicPlayer(Playlists.planetRock).playlistInformation(room).map { maybePlaylistInformation =>
  ...
  it should "report that that Pet Sounds is playing" in {
    musicPlayer(Playlists.petSounds).playlistInformation(room).map { maybePlaylistInformation =>
      maybePlaylistInformation should equal(
  ...
  val requestsAndResponses: Map[String, String] = Map(
    ...
    "favorites items 0 99999999 tags:name" ->
      """favorites items 0 99999999 tags%3Aname title%3AFavorites
        ...
        |id%3Aee3fea76.1 name%3APlanet%20Rock type%3Aaudio isaudio%3A1 hasitems%3A0 count%3A2""".stripMargin.replace('\n', ' '),
    "albums 0 5 tags:la" ->
      """albums 0 5 tags%3Ala
        ...
        |id%3A4483 album%3AGreatest%20Hits artist%3AThe%20Police""".stripMargin.replace('\n', ' ')) ++
